...
./driver.sh -r
```
- Option to run both sequentially using:
```bash
./driver.sh -sr
```
- For help, type
...
./driver.sh -n
``` 
- Option to run sequentially with setup and roi_map scripts
```bash
./driver.sh -srn
```
- For help, type
...
./driver_group.sh -sm
```
- Option to specify the name of the output subdirectory
```bash
./driver_group.sh -smo < output subdirectory >
```
...
./driver_group.sh -c
```
- Option to run sequentially with [-s] and [-m] arguments
```bash
./driver_group.sh -smc
```
- If running options sequentially, make sure you are specifying the same output directory
